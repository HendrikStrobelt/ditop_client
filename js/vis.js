...
(function() {
  var SORT_BY_CVALUE, SORT_BY_DVALUE, SORT_BY_GROUP, actualDataset, addGroupItems, allAnnotations, allClouds, bestRadius, bestRadiusScale, cloudData, configurations, createLabels, createPieBackground, decorateCloudsWithSets, decorateNewClouds, drawClouds, fill, gDragBehav, gDragDrag, getBitIndices, height, loadDataSet, moveGroupItems, moveGroupItemsDiscrete, numberFormat, offsetVisX, offsetVisY, selectedSet, selectedTopicSize, serverURL, setAttributes, showTextLabels, slotSize, slotsHorizontal, sortAndUpdate, svg, topicLabelNumberMapping, updateDataSet, updateDiTop, updateWithDValue, width, zoom, zoomInstance;
    ...
    return $.ajax({
      ...
      success: function(resData) {
        ...
        if (resData != null) {
          ...
          $("#workingSymbol").toggleClass("hidden");
          updateDiTop();
          return createLabels(resData.setNamesSorted);
        }
      }
    });
  };
  ...
  updateDiTop = function() {
    ...
    cdGroups.each(function() {
      return d3.select(this).select(".setDecoration").remove();
    });
  ...
  sortAndUpdate = function(method) {
    ...
    cdGroups = svg.selectAll(".clouds").data(cloudData, function(d) {
      return d.topicName;
    });
    ...
    return cdGroups.each(function(d, i) {
  ...
  decorateNewClouds = function(clouds) {
    ...
    clouds.append("text").classed({
      ...
      return numberFormat(d.disValue);
    });
  ...
  decorateCloudsWithSets = function(clouds) {
    ...
    for (_i = 0, _len = setAttributes.length; _i < _len; _i++) {
      ...
      clusterLabel.append("text").classed(s.name + "_textLabel", true).classed("labelText", true).attr({
        y: function(d) {
          return -Math.cos((s.sector + 1) * (Math.PI / 3)) * (bestRadiusScale(d.recommendedRadius) + 7);
        },
        x: function(d) {
          return Math.sin((s.sector + 1) * (Math.PI / 3)) * (bestRadiusScale(d.recommendedRadius) + 7);
        }
      }).style({
        ...
        return numberFormat(d.characteristicness[topicLabelNumberMapping[s.testFor]]);
      });
